...
jQuery(document).ready(function () {
  ...
  if (jQuery('select[name="_status"]').length === 0) {
    ...
    if (current_user_can_publish_posts || current_status === 'publish' && current_user_can_edit_published_posts) {
      ...
      jQuery('<span>&nbsp;<a href="#post_status" class="edit-post-status" tabindex=\'4\'>' + i18n.edit + '</a></span>' + ' <div id="post-status-select">' + ' <input type="hidden" name="hidden_post_status" id="hidden_post_status" value="in-progress" />' + " <select name='post_status' id='post_status' tabindex='4'>" + ' </select>' + '  <a href="#post_status" class="save-post-status button">' + i18n.ok + '</a>' + '  <a href="#post_status" class="cancel-post-status">' + i18n.cancel + '</a>' + ' </div>').insertAfter('#post-status-display');
  ...
  if (jQuery('select[name="post_status"]').length > 0) {
    ...
    vw_update_save_button(i18n.save);

    // Bind event when OK button is clicked
    jQuery('.save-post-status').on('click', function () {
      vw_update_save_button();
    });

    // Add custom statuses to Status dropdown
    vw_append_to_dropdown('select[name="post_status"]');
    ...
    if (!vw_can_change_status(current_status)) {
      jQuery('#post-status-select').hide();
      jQuery('.edit-post-status').hide();

      // set the current status as the selected one
      const $option = jQuery('<option></option>').text(current_status_name).attr('value', current_status).attr('selected', 'selected');
      $option.appendTo('select[name="post_status"]');
    }

    // If custom status set for post, then set is as #post-status-display
    jQuery('#post-status-display').text(vw_get_status_name(current_status));
  } else if (jQuery('select[name="_status"]').length > 0) {
    vw_append_to_dropdown('select[name="_status"]');
  ...
  if (jQuery('ul.subsubsub')) {
    vw_add_tooltips_to_filter_links('ul.subsubsub li a');
  }

  // Add custom statuses to Status dropdown
  function vw_append_to_dropdown(id) {
    ...
    if (id === 'select[name="_status"]' && current_user_can_publish_posts) {
      jQuery(id).append(jQuery('<option></option').attr('value', 'publish').text(i18n.published));
    }
    ...
    jQuery.each(custom_statuses, function () {
      ...
      const $option = jQuery('<option></option>').text(this.name).attr('value', this.slug).attr('title', this.description ? this.description : '');
  ...
  function vw_can_change_status(slug) {
    let change = false;
    jQuery.each(custom_statuses, function () {
      if (this.slug === slug) {
        change = true;
  ...
  function vw_add_tooltips_to_filter_links(selector) {
    jQuery.each(custom_statuses, function () {
  ...
  function vw_update_save_button(text) {
    if (!text) {
  ...
  if (jQuery('.post-state').length > 0) {
    vw_remove_post_title_trailing_dashes();
  }
  ...
  function vw_remove_post_title_trailing_dashes() {
